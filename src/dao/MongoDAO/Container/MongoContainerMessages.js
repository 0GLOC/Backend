    ...
    save = async(object) => {
        try {
            ...
            if (objects.length === 0) {
                ...
                let set = await this.model.updateOne({},{$push:{comments: {$each: [{author: {id: object.author.id, name: object.author.name, age: object.author.age, nickname: object.author.nickname, avatar: object.author.avatar, date: object.author.date}, text: object.text, id: lengthObj}]}}});
            }
        } catch (error) {
            console.log(error)
        };
